...
const TableOfContents = () => {
  ...
  return (
    <nav className="my-2 table-of-contents">
      ...
      <ul className="space-y-1 md:space-y-2 text-lg">
        {items.map((item) => (
          <li
            key={item.id}
            className={item.level === 3 ? "ml-4 text-base sm:text-md md:text-lg 2xl:text-base" : "text-md sm:text-lg md:text-xl 2xl:text-md"}
          >
            <a
              href={`#${item.id}`}
